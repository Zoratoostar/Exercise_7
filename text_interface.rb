...
class TextInterface
  ...
  def start_menu
    loop do
      ...
      begin
        ...
        when '4'
          # puts "  Выберите вариант:"
          # puts "  1 Заполнить объём грузового вагона."
          # puts "  2 Занять место в пассажирском вагоне."
          # puts "  Для перехода к предыдущему меню введите любое другое значение."
          # print "  "
          # case gets.strip
          # when '1'
          #   fill_in_freight_wagon
          # when '2'
          #   occupy_seat_in_passenger_carriage
          # end
          fill_or_occupy
        when '5'
          create_route
  ...
  def fill_or_occupy
    ...
    if accepting_station
      puts "   Выберите поезд:"
      accepting_station.each_train do |trn, index|
        puts "   #{index + 1}. #{trn}"
      end
      print "   "
      index = gets.to_i - 1
      ...
      if train.class == CargoTrain
        ...
        train.each_carriage do |wagon, index|
          puts "    #{index + 1}. #{wagon}"
        end
        print "    "
        index = gets.to_i - 1
        wagon = train.carriages[index] if index >= 0
        if wagon.class == FreightWagon
        ...
        train.each_carriage do |car, index|
          puts "    #{index + 1}. #{car}"
        end
        print "    "
        index = gets.to_i - 1
        car = train.carriages[index] if index >= 0
        if car.class == PassengerCarriage
  ...
  def add_stations_to_route
    ...
    if route
      ...
      stations.each do |stn|
        available_stations << stn if !(route.stations.include? stn)
      end
      loop do
